#!/usr/bin/env python3
"""Confidence sweep in the canonical 3-class space (header-footer, text-area,
footnote). Predicts once, then sweeps the confidence threshold and records
precision/recall/F1 per class at IoU 0.5, writing a CSV for later plotting.

header+footer are combined (relabel, matched individually); text-area boxes are
merged into one envelope per page (post-processing). See canon_eval.py.

Usage:
  python canon_sweep.py <weights> <dataset_dir> <remap> <out_csv> [framework] [device]
"""
...
def main() -> int:
    ...
    out_csv = sys.argv[4]
    framework = sys.argv[5] if len(sys.argv) > 5 else "rtdetr"
    ...
    for r in results:
        ...
        for c in CANON:
            if raw[c]:
                gt[c][idx] = [envelope(raw[c])] if c in MERGE_CLASSES else raw[c]
        pr = {c: [] for c in CANON}
        ...
        for c in CANON:
            ...
            if c in MERGE_CLASSES:
                preds[c].append((idx, envelope([b for b, _ in pr[c]]),
                                 max(cf for _, cf in pr[c])))
            else:
                for b, cf in pr[c]:
    ...
    with open(out_csv, "w", newline="") as f:
        ...
        for c in CANON:
            for conf in GRID:
                P, R, F1, tp, fp, fn = pr_at(gt[c], preds[c], conf)
                wr.writerow([CANON[c], conf, f"{P:.4f}", f"{R:.4f}",
                             f"{F1:.4f}", tp, fp, fn])
                if c not in best or F1 > best[c][1]:
                    best[c] = (conf, F1, P, R)
    print(f"wrote {out_csv} over {n_img} images")
    for c in CANON:
        conf, F1, P, R = best[c]
        print(f"  {CANON[c]:13} max-F1={F1:.3f} @conf {conf:.2f}  (P {P:.3f}, R {R:.3f})")
    return 0
